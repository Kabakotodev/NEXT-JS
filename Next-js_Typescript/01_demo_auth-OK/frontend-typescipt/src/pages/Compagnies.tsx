...
const Compagnies = () => {
  ...
  const updateCompagnie = async () => {
    ...
    try {
      await fetch(`${API_URL}/${editingCompagnie.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
  ...
  const handleDeleteSelected = async () => {
    if (!window.confirm(`Supprimer ${selectedItems.length} compagnie(s) ?`))
      return;

    try {
      await Promise.all(selectedItems.map(id => deleteCompagnie(id)));
      setSelectedItems([]);
      notifySuccess(
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        <div className="flex items-center gap-3">
          ...
          <h1 className="page-title">Gestion des Compagnies</h1>
            ...
            {paginatedData.map(compagnie => (
              <tr key={compagnie.id}>
                <td>
                  <input
                    type="checkbox"
                    checked={selectedItems.includes(compagnie.id)}
                    onChange={() =>
                      setSelectedItems(p =>
                        p.includes(compagnie.id)
                          ? p.filter(i => i !== compagnie.id)
                          : [...p, compagnie.id]
                      )
                    }
                  />
                ...
                <td>{compagnie.id}</td>
                <td className="font-medium">{compagnie.nomCompagnie}</td>
                <td>
                  <span className="badge badge-primary">
                ...
                <td className="flex gap-2">
                  ...
                  <button onClick={() => deleteCompagnie(compagnie.id)}>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <h2 className="text-xl mb-4">
              {editingCompagnie ? "Modifier Compagnie" : "Nouvelle Compagnie"}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input
                ...
                onChange={e =>
                  setFormData({ ...formData, nomCompagnie: e.target.value })
                }
                required
              />
              <input
                ...
                onChange={e =>
                  setFormData({ ...formData, sigleCompagnie: e.target.value })
                }
                required
              />
              <textarea
                ...
                onChange={e =>
                  setFormData({ ...formData, descCompagnie: e.target.value })
                }
              />
              <input
                className="form-input"
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    nationaliteCompagnie: e.target.value,
                  })
                }
              />
