...
// import { Edit, Key, UserCheck, UserX, Eye } from "lucide-react";
...
const AdminGestionUsers = () => {
 ...
 const handleDetailClick = (user: User) => {
  ...
  // Fonction sélectionner utilisateur 
  ...
  const handleDeleteUsers = async () => {
    ...
    const confirmed = window.confirm(
      `⚠️ Voulez-vous vraiment supprimer ${selectedIds.length} utilisateur(s) ?\n\nCette action est irréversible.`
    );
    if (!confirmed) return;
    try {
      const response = await fetch(
        `${API_BASE}/api/admin/users/delete`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
              ...
              <tr key={user.id}>
                ...
                <td className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-cyan-100 hover:bg-cyan-200 text-cyan-600 transition"
                  >
                ...
                {/* <td className="flex gap-2">
                  <button
                    onClick={() => handleEditClick(user)}
                    className="btn-warning text-xs px-3 py-1 flex items-center gap-1"
                  >
